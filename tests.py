...
def try_this(todo, run, truth, compare, *args, **kargs):
    ...
    Run a function, test the output with compare, and print and error if it doesn't work
     todo (int or str): The Todo number
    run (func): The function to run
    truth (any): The correct output of the function 
    compare (func->bool): Compares the output of the `run` function to truth and provides a boolean if correct
    *args (any): Any arguments that should be passed to `run`
    **kargs (any): Any kargs that should be passed to compare
